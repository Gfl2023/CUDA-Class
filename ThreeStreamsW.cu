...
int main()
{
	...
	for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS*3)
	{
		...
		if(i+DATA_CHUNKS+DATA_CHUNKS < ENTIRE_DATA_SET)
		{
		cudaMemcpyAsync(A2_GPU, A_CPU+i+DATA_CHUNKS+DATA_CHUNKS, DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice,Stream2);
		cudaMemcpyAsync(B2_GPU, B_CPU+i+DATA_CHUNKS+DATA_CHUNKS, DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice,Stream2);
		trigAdditionGPU<<<GridSize,BlockSize,0,Stream2>>>(A2_GPU, B2_GPU,C2_GPU,DATA_CHUNKS);
		cudaMemcpyAsync(C_CPU+i+DATA_CHUNKS+DATA_CHUNKS,C2_GPU,DATA_CHUNKS*sizeof(float),cudaMemcpyDeviceToHost,Stream2);
		}
	}
